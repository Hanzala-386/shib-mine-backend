...
- **Who pushes:** `git commit` is blocked in the sandbox but plain `git push github main` WORKS (verified Jul 18 2026). Flow: agent finishes task (platform auto-commits at checkpoint) → next turn agent (or user) pushes → Railway builds ~2-4 min. **Auto-deploy can silently stall**: on Jul 18 2026 a push landed on remote main but prod still served old code 15+ min later — always verify with a probe of a new route; if stale, the user must open the Railway dashboard and trigger a manual redeploy (agent has no Railway access).
