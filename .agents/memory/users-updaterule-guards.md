...
**Companion step — remove client self-write fallbacks:** guarding a field breaks any client PB-SDK fallback that self-writes it (silent 403 at runtime). When adding a guard, grep the client for writes to that field and convert those paths to server-only (fail closed with a retry message rather than a client-side balance mutation). `hit_tickets` guarded Jul 2026: the redeem PB fallback in WalletContext was removed for exactly this reason — ticket mint/spend is Express-only.
