...
**Payout must equal the displayed score:** the server's per-hit tally UNDERCOUNTS honest play (bridge drains queued hits faster than the server's min-spacing guard; hits before the WS handshake never arrive), and a server-clock elapsed clamp caps honest scores when the WS session starts late relative to real gameplay. GAME_OVER must therefore reconcile UP to the client score, bounded only by rate cap (15 pts/s vs the LARGER of client/server elapsed, clamped to the session hard limit) + absolute PT cap + impossible-rate blacklist. Per-hit counting is fraud telemetry, never the payout source. **Why:** paying less than what the player's screen showed (65 shown → 45 paid → 2× paid 90 not 130) destroys user trust; the owner explicitly prioritized payout accuracy over anti-cheat strictness.
