function CGame(oData){
    ...
    // ── ARCADE PvP adapter state ──────────────────────────────────────────
    var _bArcadeFrozen = false;       // host froze us OR we already reported out
    var _bArcadeStarted = false;      // player tapped through the help panel
    var _oArcadeReportTimer = null;   // throttle handle for score reporting

    // Throttled cumulative-score reporter (~600ms): guarantees the server's
    // 500ms min-interval window always elapses between SCORE messages, no
    // matter how fast fruits are sliced. Sends the CURRENT total; the SDK and
    // server both work off cumulative scores.
    var _fArcadeReportScore = function(){
        if(!(window.Arcade && Arcade.isMatch())){ return; }
        if(_bArcadeFrozen){ return; }
        if(_oArcadeReportTimer !== null){ return; } // report already pending
        _oArcadeReportTimer = setTimeout(function(){
            _oArcadeReportTimer = null;
            if(_bArcadeFrozen){ return; }
            Arcade.onScore(_iScore);
        }, 600);
    };
    ...
    this._init = function(){
        ...
        _iNumFruitsSpawn = 0;
        _iLives = (window.Arcade && Arcade.isMatch()) ? Arcade.maxLives(NUM_LIVES) : NUM_LIVES;
        ...
        // ARCADE PvP: sync the lives HUD with the server-authoritative count and
        // register the host freeze handler (fired when the match ends remotely).
        if(window.Arcade && Arcade.isMatch()){
            _oInterface.loseLife(_iLives);
            Arcade.onFreeze(function(){
                _bArcadeFrozen = true;
                _bUpdate = false;
                if(_oArcadeReportTimer !== null){ clearTimeout(_oArcadeReportTimer); _oArcadeReportTimer = null; }
            });
            // The server-authoritative lives count arrives with ARCADE_MATCH_START,
            // which can land AFTER _init (the SDK's ?arcade=1 flag makes isMatch()
            // true at page load, before the postMessage handshake). Re-sync the
            // lives + HUD when it lands — but only while the player is still on the
            // TAP-TO-START help panel, so a re-sent handshake can never restore
            // lives mid-game.
            window.addEventListener('arcade:matchstart', function(e){
                if(_bArcadeStarted || _bArcadeFrozen){ return; }
                var iLives = e && e.detail && Number(e.detail.lives);
                if(iLives > 0 && iLives !== _iLives){
                    _iLives = iLives;
                    _oInterface.loseLife(_iLives);
                }
            });
    ...
    this.exitFromHelp = function(){
        _bUpdate=true;
        // ARCADE PvP: dismissing the help panel is the TAP-TO-START moment —
        // tell the host gameplay began (clears the AFK forfeit timers).
        if(window.Arcade && Arcade.isMatch()){
            _bArcadeStarted = true;
            Arcade.onStart();
        }
    };
    ...
    this.gameOver = function(){  
        ...
        // ARCADE PvP: report the final score once and hold the frame — the RN
        // host owns the result overlay, so no local end panel / restart here.
        if(window.Arcade && Arcade.isMatch()){
            if(_oArcadeReportTimer !== null){ clearTimeout(_oArcadeReportTimer); _oArcadeReportTimer = null; }
            this.removeAllLines();
            if(!_bArcadeFrozen){
                _bArcadeFrozen = true;
                Arcade.onPlayerOut(_iScore);
            }
            return;
        }
    ...
    this.onExit = function(){
        // ARCADE PvP: the RN host owns all navigation during a match — never
        // tear the game down into the local menu.
        if(window.Arcade && Arcade.isMatch()){
            return;
        }
        this.unload();
