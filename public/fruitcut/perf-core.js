...
(function () {
  ...
  /* ══ GAME-SPECIFIC GLUE (Fruit Cut / CTL-CreateJS) ══════════════════ */

  // Gameplay-active gate: s_oGame is a CTL global, non-null only during play.
  window.__perfActive = function () { return !!window.s_oGame; };

  // Immediate scale apply: sizeHandler() re-derives the canvas backing store
  // from the PERF-patched Android branch in ctl_utils.js.
  window.__perfApplyScale = function () {
    try {
      if (window.s_oStage && typeof window.sizeHandler === 'function') window.sizeHandler();
    } catch (e) { /* stage not booted yet */ }
  };
})();
