...
(function () {
  ...
  /* ══ GAME-SPECIFIC GLUE (Color Rush / CreateJS-CTL) ═════════════════════ */

  // Gameplay-active gate for telemetry: only count FPS while on the game page.
  // curPage is a top-level `var` in canvas.js → a window property.
  window.__perfActive = function () { return window.curPage === 'game'; };

  // Immediate scale apply: resize the canvas backing store + stage transform
  // NOW (the PERF-patched lines in main.js/canvas.js keep resize/orientation
  // paths consistent afterwards). stageW/stageH/stage are later globals —
  // guarded because a scale step can fire before the game canvas exists.
  window.__perfApplyScale = function (s) {
    var c = document.getElementById('gameCanvas');
    if (!c || typeof window.stage === 'undefined' || !window.stage) return;
    var sw = Number(window.stageW), sh = Number(window.stageH);
    if (!isFinite(sw) || !isFinite(sh) || sw <= 0 || sh <= 0) return;
    var eff = (window.devicePixelRatio || 1) * s;
    c.width = Math.max(1, Math.round(sw * eff));
    c.height = Math.max(1, Math.round(sh * eff));
    window.stage.scaleX = window.stage.scaleY = eff;
  };
})();
