...
import {
  ...
  type TableState, type ShotInput, type FrameSample,
...
export default function PoolMatchScreen() {
  ...
  const animateFrames = useCallback((frames: FrameSample[], onDone: () => void) => {
  ...
  /* ── practice resolution (local rules) ────────────────────────────────── */
  const resolvePractice = useCallback((res: ReturnType<typeof simulateShot>) => {
    stateRef.current = res.finalState;
    if (res.pocketed.includes(8)) {
      if (res.cuePocketed) { setWinner('B'); setWonTickets(0); } // scratch on the 8 = loss
      else { setWinner('A'); setWonTickets(cfg.winnerTickets); }
      syncDisplay();
      setMode('gameover');
      stopClock();
      return;
    ...
    syncDisplay();
  }, [cfg.winnerTickets, stopClock, syncDisplay]);
  ...
  const shoot = useCallback((angle: number, power: number) => {
    ...
    if (modeRef.current === 'practice') {
      ...
      animateFrames(res.frames, () => resolvePractice(res));
  ...
  const turnLabel = isPractice ? 'Practice' : myTurn ? 'Your shot' : `${opponentName}'s shot`;
  ...
  return (
    <View style={[styles.root, { paddingTop: insets.top + webTop }]}>
      ...
      >
        {proj && <PoolTable proj={proj} balls={displayBalls} aimPath={aimPath} showGhost={!!aim} />}
