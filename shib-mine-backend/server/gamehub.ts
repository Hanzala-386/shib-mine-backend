...
/** DELETE via admin token. pbHttp resolves {raw:''} on a 204 (empty body). */
export async function pbDelete(path: string) { return pbHttp('DELETE', path, null, await getAdminToken()); }
/** DELETE that THROWS unless PB removed the record. Used as an atomic single-winner CLAIM:
 *  concurrent callers race and only ONE DELETE succeeds (the others get a 404 {code}). */
export async function pbDeleteChecked(path: string): Promise<void> {
  const r = await pbDelete(path);
  // A genuine 204 resolves to exactly {raw:''}. An error JSON → {code,...}; a non-JSON proxy/CDN
  // error page (e.g. 502 HTML) → {raw:'<html>…'}. Treat anything but an empty body as a failure so
  // a claim never "succeeds" against a record that still exists (which would enable a re-refund).
  if (r && (r.code || (typeof r.raw === 'string' && r.raw !== ''))) {
    throw new Error(`PB DELETE failed (${path}): ${JSON.stringify(r).slice(0, 200)}`);
  }
}
