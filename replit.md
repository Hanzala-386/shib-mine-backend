...
- **Session 10b — VERSION-AWARE gate (P1 hotfix)**: old APKs (old bridge, no `v` in GAME_START) were blocked by the hard gate → "Connection failed". Now: new bridge.js sends `GAME_START {v:2}` → strict hard gate (row confirmed before SESSION_READY). Legacy clients (no `v`) → SESSION_READY sent IMMEDIATELY (old behavior), match row created async best-effort; `wsCommitSession` has a **legacy-only** fallback INSERT if that write never lands; a late async insert landing after session end is DELETED (stray-duplicate race guard). Strict path unchanged; claim correlation unchanged (works for both). E2E 17/17. bridge-upload.zip regenerated (v:2 flag) — re-upload to webcod.in `/arcade/`.
