...
- **Client (`app/solo-play.tsx`)**: `claimLockRef` double-click lock on Claim/2× (unlocks on failure), Android back-nav blocked except reward/error/idle states; `lib/api.ts` game reward/ad-token calls pass `matchId`.
