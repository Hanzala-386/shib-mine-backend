...
- **Ads = AdMob ONLY** (`lib/AdService.ts` ADMOB_AD_UNIT_IDS — Android = LIVE unit IDs under AdMob app ca-app-pub-7314448641809053; iOS = Google test IDs; admin-panel IDs stored but unused). **app.json `androidAppId` is still the Google TEST ~App ID — live ads will NOT fill in an APK until the user's real `ca-app-pub-7314448641809053~…` App ID is set there.** `lib/admob.ts` = lazy-require SDK gateway (web stub via metro.config.js redirect → `lib/admobWebStub.ts`; Expo Go → simulation). `context/AdContext.tsx`: preloads interstitial+rewarded on init, show* waits ≤6s for load then proceeds WITHOUT ad (never blocks claims); rewarded resolves true only on EARNED_REWARD.
...
## Session 13 — Single-Session Security + Knife Hit Banner + Color Rush 60FPS + RNGMA exact-pin
- **Single-session enforcement (client-side, fail-open)**: new `users.session_token` text field (boot migration `ensureSessionTokenField` in BOTH server copies, chained after `ensureBlacklistFields`; confirmed live in shared PB). Login claims the session: `claimOrEnforceSession` in `context/AuthContext.tsx` — no local token → generate `sess_…` + PATCH (self-update) → store locally ONLY after PATCH succeeds; empty remote (pre-migration) → re-claim with local; mismatch → `forceSessionLogout` (foreground alert + signOut; clears LOCAL token only, NEVER the PB field). Watcher useEffect: PB realtime subscribe on own record + 45s poll + AppState foreground re-check. **Invariants: never logout on read errors or empty remote token (no lockouts offline); a restarting stale device signs out, it does NOT steal the session back.** Realtime needs the `react-native-sse` EventSource polyfill (native-only, in `lib/pocketbase.ts`) — verified e2e vs live PB: second-device PATCH pushed via SSE in 103ms. Poll is the safety net if SSE misbehaves in the APK (polyfill untestable in sandbox). Score calculation untouched. Enforcement is UI-level — money routes have no session_token check (deliberate; add server-side if ever needed).
- **Knife Hit banner** (`app/solo-play.tsx`): game area wrapped in flex:1 `gameArea` View; `bannerBar` below (minHeight `BANNER_HEIGHT` + insets.bottom, always reserves space on native → zero layout shift). `AdMobBanner` (now exported from `StickyBannerAd.tsx`, 60s hard-refresh + blur/background unmount inside) mounts only while `phase==='game' && !gameError` — unmounts under summary/reward modals (AdMob viewability).
- **Color Rush frame-rate independence** (`public/color/js/canvas.js`, CRLF — patch via node script, not edit tool): Ticker RAF timingMode (feature-guarded) + fixed-timestep accumulator in `tick()` (STEP_MS=1000/60, MAX_STEPS=4, event.delta clamped); `updateGame()` steps N×, `stage.update()` once. `game.js` uses no event.delta → no double-count. All 6 arcade games now frame-rate independent. GAME_HOSTS color `v:1→2` (cache bust).
- **Config sync**: `react-native-google-mobile-ads` pinned EXACT `"16.1.0"` (package.json + lockfile) matching the user's stable APK; lockfile firewall URLs rewritten → registry.npmjs.org (Railway npm ci). **No `kspVersion` added** — expo-build-properties 1.0.10 schema (ajv) hard-fails on it; KSP is auto-derived from `kotlinVersion: 2.2.20` via Expo's KSPLookup table (Session 12).
- **Deploy**: upload `dist/color-arcade.zip` → webcod.in `/color/`; Railway backend redeploy (`git push github main`) for the session_token migration in the prod bundle (field itself already live in shared PB).

## Session 14 — Device-Agnostic Performance Pass (all 7 games)
- **Already in place (audited, no change needed)**: fixed-timestep/delta-time in ALL games (Flappy custom accumulator, Fruit Cut CMain accumulator, Color Rush accumulator, Knife Hit `/arcade` + Stack/2048/Ice Block = Construct 3, natively dt-based); C3 exports ship texture atlases; CTL games fully preload via CPreloader/howler; webcod.in already serves `Cache-Control: public, max-age=604800` on all assets.
- **Knife Hit solo = `public/arcade/` (C3 export, useWorker=!1, bridge.js)** — NOT the legacy `public/game/Knife hit Template/` dir (assets only, no JS).
- **Applied**: (1) Fruit Cut `CMain.js` — `createjs.Ticker.timingMode = RAF` (was default setTimeout; vsync pacing on WebViews); (2) Color Rush `canvas.js` — `stage.enableMouseOver(20)` gated to non-touch devices only (was burning 20 display-list hit-test scans/sec on phones for desktop-hover cosmetics); (3) `.htaccess` in all 7 game folders (IfModule-guarded: media 30d, js/css/json 300s must-revalidate, html no-cache — if the host 500s, delete the .htaccess, it's additive only).
- **CRITICAL cache-bust bug found+fixed**: `color/index.html` loaded `js/canvas.js` with NO `?v=` while the host caches js 7 DAYS → Session-13's accumulator patch would never reach devices with a warm cache. Now `canvas.js?v=2`. Rule: **any patched game JS file MUST get a `?v=` bump in its own script tag** — bumping GAME_HOSTS only busts index.html, not the js it references. Fruit Cut `CMain.js?v=2→3`; GAME_HOSTS fruitcut v2→3 (color stays v2, never deployed).
- **Explicitly DECLINED (with architect)**: OffscreenCanvas/worker rendering — C3 worker mode breaks all main-thread `runOnStartup`/globalVars score adapters + bridge.js (documented constraint), CreateJS has no OffscreenCanvas path; texture re-atlasing of CTL sprites — canvas2D has no per-texture draw-call cost, load-time-only benefit vs rewriting every `getResult` call site; object pooling — spawn rates are 1–3 objects/sec (not a GC bottleneck), CTL classes are closure-based with no re-init and fruit lifecycle is coupled to scoring; breakage risk > gain. Score paths byte-untouched (grep-verified).
- **Deploy**: upload ALL 7 zips from `dist/` (`arcade-arcade.zip` → `/arcade/`, others map by name). fruitcut + color contain JS changes; the other 5 are .htaccess-only.

## Session 15 — Color Rush Consistency Pass (same-device speed/smoothness variance)
- **Root cause found**: `renderWorld()` rebuilds the ENTIRE road display list per call — `worldContainer.removeAllChildren()` + ~150 segments × 3-4 `new createjs.Shape` (renderPolygon) + `.clone()` per sprite (renderSprite), drawDistance=150. It ran inside `updateWorld()` per FIXED STEP → during accumulator catch-up a lagging device rebuilt the display list up to 4× per rendered frame (GC-churn death spiral: lag→more work→more lag).
- **Fix (logic/render split)**: `updateWorld()` now runs `updateSprites()` only; `canvas.js tick()` calls `renderWorld()` ONCE per rendered frame (`if (steps > 0 && typeof renderWorld === 'function')`). Safe: updateSprites' collision logic reads lane offsets, never renderWorld's screen projections; ball-slide tweens inside renderWorld are TweenMax time-based (own clock). Score paths untouched (`playerData.score++` in updateSprites; `__arcadeColorTick` in updateGame).
- **Memory leak fixed (sound.js)**: `playSound()` stored every SFX instance in `$.sound[id]` forever (complete handler spliced `soundPushArr` but never freed the map entry; id increments monotonically) → one retained SoundInstance per ball collected. Now `delete $.sound[thisSoundID]` on complete. Music/loop paths already null out.
- **GPU/hardware accel**: WebView `androidLayerType="hardware"` + `overScrollMode="never"` on BOTH game WebViews (`arcade-match.tsx` — all 6 PvP games; `solo-play.tsx` — Knife Hit solo). Canvas GPU-promotion CSS in color/index.html: `#gameCanvas{will-change:transform;transform:translateZ(0);backface-visibility:hidden;}`.
- **FPS meter (canvas.js `_fpsSample`)**: DOM overlay div (top-left, monospace, green ≥50 / amber ≥30 / red <30), updated every 500ms from real RAF tick cadence, `pointer-events:none`, zero canvas cost. Always-on for now — remove or gate after diagnosis.
- **Preload audit result**: NO lazy loads — all sprites/themes/sounds are in the LoadQueue manifest (loader.js), createjs.Sound plugin installed pre-load. Jitter was GC churn, not asset fetches.
- **Restart-path audit result**: listeners registered once at build (top-level); `prepareWorld` removeAllChildren; `stopGame` TweenMax.killAll; `gameData.explodes` reset in startGame; explode shapes spliced at alpha<0 — no accumulation on restart.
- **Versions**: color/index.html — `canvas.js?v=3`, `game.js?v=2` (was unversioned), `sound.js?v=2` (was unversioned); GAME_HOSTS color v2→3. `dist/color-arcade.zip` rebuilt (Jul 17). CRLF rule: patch color js via node scripts, never the edit tool.

## Session 16 — Telegram "Share Contact" Phone Verification for KYC (replaces MSG91 WhatsApp OTP)
- MSG91 WhatsApp OTP is FULLY REMOVED (routes, api client fns, OTP UI, `@msg91comm/sendotp-react-native` package uninstalled). Phone verification is now via Telegram bot contact sharing — zero per-message cost, no OTP typing.
- **Flow**: Client (`app/verify-account.tsx`): "Verify via Telegram" button (`kyc-tg-btn`, Telegram-blue #229ED9, paper-plane icon) → `api.startTelegramVerification({pbId, identifier})` → server mints one-time token → app opens `https://t.me/<bot>?start=TOKEN` → waiting card (spinner + "Open Telegram again" + Cancel, `kyc-tg-waiting`) → app polls OWN PB user record every 2s (direct PB SDK — works on APK without Express) for `wa_verified_phone === digits(dial+phone)` → green chip "Phone number verified via Telegram". 5-min poll timeout; editing the phone cancels the wait. Verified state still DERIVED: `waIdentifier === digits(dial+phone)`.
- **Server** (BOTH routes.ts copies, byte-identical): `POST /api/app/verification/telegram/start` — validates user, rate-limits (5s gap, 12/hour), writes PB `telegram_verifications` row `{token, user, phone, status:pending}` (15-min TTL, token = 32-hex crypto random), returns `{token, botUsername, deepLink}`. `POST /api/app/telegram/webhook` — replies 200 FIRST; checks `X-Telegram-Bot-Api-Secret-Token` header (sha256 of bot token, set at registration); `/start TOKEN` → replies with `request_contact` keyboard ("📱 Share Contact to Verify Account"); contact message → anti-spoof `contact.user_id === from.id` (rejects forwarded contacts) → digits-compare Telegram phone vs session phone → match: patch `users.wa_verified_phone/_at` + mark session `verified` + ✅ reply; mismatch: ❌ reply with masked expected number. Sessions in PB (not memory) — survive restarts.
- **Bot config**: `TELEGRAM_BOT_TOKEN` env (NO hardcoded fallback — empty token skips webhook registration with a boot warning), `TELEGRAM_BOT_USERNAME` (default `Shibahit_Bot`). Webhook auto-registered on boot via `registerTelegramWebhook()` → base = `TELEGRAM_WEBHOOK_BASE` || `https://$REPLIT_DEV_DOMAIN` || `https://backend.webcod.in`. **PROD OWNS THE WEBHOOK (Jul 18)**: dev NEVER registers unless `TELEGRAM_DEV_WEBHOOK=1` (or `TELEGRAM_WEBHOOK_BASE`) is set — the Replit dev domain sleeps when the workspace is idle, so a dev-owned webhook 404s every real user attempt (this was the "silent bot" root cause: Telegram logged `Wrong response from the webhook: 404 Not Found`). Webhook manually pointed at `https://backend.webcod.in/api/app/telegram/webhook`; Railway re-registers the same URL+secret on every boot once the token env is set.
- **⚠ BOT TOKEN REQUIRED**: the token the user first provided (`8850283260:…`) fails getMe with 401 (invalid) and was NOT baked into code — a valid bot token from @BotFather must be set as secret `TELEGRAM_BOT_TOKEN`; until then boot logs "webhook registration skipped" and verification cannot complete (start route + deep link still work).
- **Schema**: `telegram_verifications` collection auto-created on boot (`ensureTelegramVerificationsCollection`, admin-only rules). **BUG FIXED (Jul 18)**: create payload used the `fields:` key, but this PB is pre-v0.23 → needs `schema:`; create 404'd silently on every boot while logging "created ✓" (no error check). Both copies now use `schema:` + check `created.code`; collection verified live in PB and `/telegram/start` returns 200 with token for the real test user; users `wa_verified_phone`/`wa_verified_at` + verification_requests `phone_verified` unchanged from MSG91 era — submit route still computes `phone_verified = wa_verified_phone === digits(countryCode+phone)` (server stays SOFT for old APKs). **SECURITY (architect-mandated)**: users updateRule now includes `wa_verified_phone:isset = false && wa_verified_at:isset = false` guards — without them any authenticated client could forge the verified phone via PB self-update and bypass Telegram entirely. The webhook writes these fields via admin token (bypasses rules); the app only READS them when polling.
- **Admin** (`app/admin.tsx`): badge now "Telegram verified" (blue, paper-plane) / "Phone not verified" (grey).
- **Deploy status (Jul 18)**: Railway deployed the telegram routes overnight (prod `/telegram/start` returns 200 + token for the real test user). **⚠ BLOCKING: Railway env is MISSING `TELEGRAM_BOT_TOKEN`** — verified by a signed fake-update test against the prod webhook: prod answered 200 but did NOT bind chat_id (secret mismatch from empty token → update silently ignored, bot can't sendMessage). User must add `TELEGRAM_BOT_TOKEN` (same value as the Replit secret) to Railway → Variables and redeploy; after that boot, prod self-registers the webhook and the full flow works even when Replit dev is asleep. `/telegram/start` works without the token (only mints a PB session + deep link).

## Session 10 — Hard-Performance Override (arcade FPS rescue)
- **RN ad gating**: `arcade-match.tsx` banners render ONLY during the VS/matchmaking screen (`showBanners = showVS`); NO ad loads during active gameplay. Game-over overlay keeps its own banner slot — do NOT add `gameover` to `showBanners`.
- **FPS telemetry**: games report `ARCADE_PERF {avgFps,minFps,renderScale}` every 10s while playing (via `Arcade.onPerf` in arcade-sdk.js — all 6 copies byte-identical); RN buffers last report in `perfRef`, writes ONE `perf_logs` row per session (PB-direct create, `expo-device` model) on gameover + unmount guard. `perf_logs` collection auto-created on boot in BOTH routes.ts copies (pre-v0.23 `schema:` key; createRule auth-only).
- **perf-core.js per game** (loaded right after arcade-sdk.js): caps devicePixelRatio via `defineProperty` getter (2, or 1.5 if `deviceMemory≤3`); rAF FPS sampler gated by per-game `__perfActive`; adaptive resolution ladder ×1→×0.75→×0.5 (<30 FPS for 2s steps down, >50 for 10s steps up); `?perfsim=<ms>` busy-wait + `?fpsdebug=1` overlay for testing.
- **Ladder wiring (CreateJS/canvas games)**: color (canvas+stage resize; main.js/canvas.js/game.js multiply by `__renderScale`), flappy (resize event → resizeCanvas re-derives), fruitcut (`sizeHandler()`; Android branch backing = CANVAS_W/H×rs + stage.scale=rs, CGame divides stageX/Y by rs — EaselJS-NEXT maps pointer to backing px; desktop branch backing×rs + CSS pin + `s_iScaleFactor×rs` absorbs input).
- **C3 trio (stack/2048/iceblock)**: STATIC dpr cap only (`LADDER=[1]` in their perf-core) — C3 runtime re-reads the capped devicePixelRatio getter on resize; adapters set `window.__c3Active` every tick (Game layout + !gameover) as the telemetry gate.
- **CRLF landmine**: color/fruitcut html+js are CRLF — the edit tool corrupts them; patch via python `io.open(newline='')` with exact-count asserts.
- **Deploy**: all six `dist/<game>-arcade.zip` rebuilt → manual upload to webcod.in; GAME_HOSTS bumped (flappy 8, fruitcut 4, stack 4, 2048 2, iceblock 2, color 4); VPS backend redeploy (scp server.cjs) needed for prod `perf_logs` creation.
- Verified: color ladder stepping via screenshot (`?perfsim=40&fpsdebug=1` → "24 FPS ×0.75"); flappy/fruitcut load clean with overlay; C3 games can't render in headless preview (WebGL missing — environment, not a regression). Native APK FPS behavior = device-only verification.

## Session 17 — Stability/UX/Compliance Batch (Jul 18 2026)
- **Stack score desync FIXED (2 root causes, both `shared/arcade.ts` copies + `arcadehub.ts` byte-identical)**: (a) stack `scoreDelta.maxIncrement` 5→60 — real C3 per-block gain is ~10 pts (NOT +1), so the old budget sat BELOW honest pace and the server-accepted score (opponent display + settlement input) crawled behind real play; (b) `acceptScore` budget clock is now remainder-preserving — advances `lastScoreAt` by whole windows consumed instead of resetting to `now` (old reset discarded ~17% of honest budget at the 600ms report cadence). Verified via `.local/t101-repro.mjs` simulation.
- **Back-button lockdown** (`arcade-match.tsx`): while `matchLive` (matchfound/playing, not practice) Android `BackHandler` swallows hardware back + `beforeRemove` blocks iOS swipe-back/nav removal; released on gameover/queue/error so explicit exits still work. Solo knife-hit unaffected (its claim flow already gates exit).
- **C3 trio dynamic resolution ladder** (`public/{stack,2048,iceblock}/perf-core.js`): dpr getter now returns `cap × __renderScale`, `__perfApplyScale` dispatches a synthetic `resize` so the C3 runtime re-reads the capped dpr and resizes the backing store; `LADDER=[1,.75,.5]` (was static `[1]`). Zips rebuilt; GAME_HOSTS bumped stack v5 / 2048 v3 / iceblock v3.
- **Game History (T104)**: new PB collection `game_history` (auto-created on boot, BOTH routes.ts copies via `ensureGameHistoryCollection`, pre-v0.23 `schema:` key; rules: list/view/create user-scoped, update/delete null → client-written append-only). `lib/gameHistory.ts` (`logGameHistory`/`fetchGameHistory`, fire-and-forget, never blocks money paths) wired into solo-play.tsx (knife-hit result), arcade-match.tsx (MATCH_RESULT: win→ticketsWon, lose→tokensLost=tier, draw→0/0), WalletContext.redeem (outcome:'redeem', tokensLost=tickets, shibWon). Read-only screen `app/game-history.tsx` (Sr|Game|Result|Won/Lost table, pull-to-refresh) + "Game History" button on games tab.
- **Dev-proxy gotcha FIXED**: Metro pathFilter excluded `startsWith('/game')` which swallowed the new `/game-history` app route → "Cannot GET". Both `server/index.ts` copies now exclude exactly `/game` + `/game/*`.
- **Notification modal** (`app/notifications.tsx`): rows tappable → full-screen scrollable modal with complete title/body/date.
- **Legal hardening**: `privacy.tsx` 16 sections / `terms.tsx` 19 sections ("Last Updated: July 18, 2026") — explicit NOT-gambling + no-deposits clauses, fraud/multi-account enforcement (suspend/ban/blacklist), restricted territories Iran/Ukraine/Afghanistan/North Korea. `TermsGateModal.tsx` TC_CONTENT = 8-section summary of same; scroll fix — Confirm enables only after true bottom reach (tolerance-based onScroll).
- **Deploy needed**: upload rebuilt `dist/{stack,2048,iceblock}-arcade.zip` to webcod.in; push backend (`git push github main` → Railway) for the acceptScore/spec fix + `game_history` collection creation in prod.

## Session 11 — Arcade Idle-Lock Settle Fix + Live AdMob IDs + Ticket Hardening
- **"Waiting for Opponent" hang FIXED** (`server/arcadehub.ts` + `shared/arcade.ts`, BOTH copies byte-identical): seat-locking unified in `lockSeat()`; new `ARCADE_WAITING_IDLE_MS=45s` idle-lock — once one seat is locked, the alive seat must make ACCEPTED score progress or is locked at current server score (idle-lock ≠ forfeit: higher score still wins). Disconnect while opponent already locked → INSTANT lock+settle (30s grace preserved when both mid-run). RESUME with opponent locked re-arms the idle timer. Settlement stays exactly-once (settled latch + p.locked guards on every timer path).
- **Live Android AdMob unit IDs** in `lib/AdService.ts` (`ADMOB_AD_UNIT_IDS`; banner /4666652323, rewarded /7506671725, interstitial /2310185360); iOS stays on Google test IDs. **BLOCKER: app.json `androidAppId` still Google TEST ~App ID — needs the real `ca-app-pub-7314448641809053~…` App ID before the release APK, else live units won't serve.**
- **hit_tickets hardening**: users updateRule gains `@request.data.hit_tickets:isset = false` (BOTH routes.ts copies; verified applied on live PB) — clients can no longer mint/move tickets via PB SDK. Client redeem PB-fallback REMOVED from `context/WalletContext.tsx` (fails closed with retry message); redeem/credit are Express-admin-token-only. Known accepted risk: `power_tokens`/`shib_balance` remain client-self-writable (required by APK fallback flows: mining claim, referral crediting, game rewards); backstop = manual withdrawal approval + blacklist checks.
- **Deploy needed**: `git push github main` → Railway (~2-4 min) for arcadehub/spec/rule changes; new APK build for AdService + WalletContext changes (after App ID lands in app.json).
