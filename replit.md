...
## Session 16 — MSG91 WhatsApp OTP for KYC (Verify Account)
- KYC submit now REQUIRES WhatsApp-OTP verification of the entered phone (new-client only; server stays SOFT for old APKs — absence/mismatch just stamps `phone_verified=false`, never blocks). Admin still approves manually.
- **Flow is SERVER-DRIVEN** (v2 — MSG91's `DefaultWidget` popup REMOVED after "Unable to Load / Failed to load widget configuration": `getWidgetProcess` returns 401 for the provided credentials; server-driven flow bypasses widget config entirely AND keeps the MSG91 key out of the APK). Client (`app/verify-account.tsx`): "Verify on WhatsApp" button → `api.sendWhatsAppOtp({pbId, identifier})` → inline OTP box appears under the phone field (code input + Verify btn + 30s resend cooldown; `otpSentTo` pins the box to the number the code went to — editing phone hides it). `api.verifyWhatsAppOtp({pbId, reqId, otp})` → green chip. Verified state DERIVED: `waIdentifier === digits(dial+phone)`.
- **Server** (BOTH routes.ts copies): `POST /api/app/verification/send-otp` — calls MSG91 widget REST `sendOtpMobile` (`MSG91_WIDGET_ID`/`MSG91_AUTHKEY` consts, env overrides win); success `message` = reqId, stored in-memory `msg91OtpSessions` reqId→{identifier,pbId} (10-min TTL); rate-limit 25s gap + 6/hour per user. `POST /api/app/verification/verify-otp` — checks session (expiry + pbId match), calls `verifyOtp {reqId, otp}`; identifier comes from the SERVER-side session, never the client (spoof-proof by construction). Writes `users.wa_verified_phone/_at`. Submit route computes `phone_verified = wa_verified_phone === digits(countryCode+phone)`; status+admin mappings emit `phoneVerified`. Sessions are in-memory: a server restart mid-entry ⇒ "code expired, resend".
- **⚠ CREDENTIALS INVALID (as of Jul 17, 2026)**: authkey/tokenAuth `551552A2j257QU6a5a8beeP1` is rejected by ALL MSG91 endpoints (401/418 AuthenticationFailure — send, verify, getWidgetProcess, balance). OTPs CANNOT be delivered until the user supplies the real widget **Token Auth** (MSG91 dashboard → OTP → widget → Integration) — set as secret `MSG91_AUTHKEY` (server env override already wired; `MSG91_WIDGET_ID` overridable too).
- **Schema** (auto-ensured on boot): users `wa_verified_phone`,`wa_verified_at` (text); verification_requests `phone_verified` (bool) via `ensureRequestPhoneVerifiedField` (REFETCHES collection — stale schema would revert the status SELECT conversion). Boot logs: `KYC fields added: wa_verified_phone, wa_verified_at ✓` + `phone_verified field added ✓`.
- **Admin** (`app/admin.tsx`): verification card shows green "WhatsApp verified" / grey "No WhatsApp OTP" badge under the Phone row.
- **Deploy**: VPS backend (backend.webcod.in) needs redeploy (rebuild server.cjs from shib-mine-backend + scp) for the route to exist in prod — until then APK OTP verify calls fail (robustPost error surfaces inline; submit stays blocked only in new clients).
