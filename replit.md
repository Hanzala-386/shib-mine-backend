...
- **Client** (`app/verify-account.tsx`): `DefaultWidget` from `@msg91comm/sendotp-react-native` (pure-JS modal, Expo Go safe; widgetId+tokenAuth constants in file). Green "Verify on WhatsApp" button under phone field → widget → `onCompletion.message` = one-time access-token → `api.verifyWhatsAppOtp({pbId, accessToken, identifier})`. Verified state is DERIVED: `waIdentifier === digits(dial+phone)` — editing phone/country auto-revokes the green chip. Mismatch shows inline error.
- **Server** (BOTH routes.ts copies): `POST /api/app/verification/verify-otp` — verifies token via `control.msg91.com/api/v5/widget/verifyAccessToken` with `MSG91_AUTHKEY` (env override wins → hardcoded fallback). The verified number is resolved from MSG91 data ONLY (verify-response fields → JWT payload); the client-sent identifier is NEVER trusted (spoof vector: valid own-number token + someone else's number) — no MSG91-side identifier ⇒ 400 with diagnostic key logging. Writes `users.wa_verified_phone/_at`. Submit route computes `phone_verified = wa_verified_phone === digits(countryCode+phone)` on the request row; status+admin mappings emit `phoneVerified`. NOTE: widget tokenAuth (in APK) == server authkey by MSG91 widget design — rotate via env `MSG91_AUTHKEY` if OTP-credit abuse appears.
