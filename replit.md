...
- **Deploy**: prod = Railway at backend.webcod.in (`server: railway-hikari`). Code pushed to GitHub main (49c9904) but Railway auto-deploy DID NOT pick it up (route still 404 15+ min later) — user must open the Railway dashboard and trigger a manual redeploy. Set `TELEGRAM_BOT_TOKEN` (+ `TELEGRAM_WEBHOOK_BASE=https://backend.webcod.in`) in Railway env for prod to own the webhook. Note: `/telegram/start` itself works WITHOUT the token (only mints a PB session + deep link); the webhook owner (currently the Replit dev server) completes verification via the shared PB.
